...
const App = () => {
  ...
  const samples = [
    { label: "Supplier: INV-A-001 (Cold Start)", data: { id: "INV-A-001", vendorName: "Supplier GmbH", date: "2024-01-12", totalAmount: 2975.0, currency: "EUR", lineItems: [{ description: "Widget", quantity: 100, price: 25.0, total: 2500.0, sku: "WIDGET-001" }], rawText: "Rechnungsnr: INV-2024-001\nLeistungsdatum: 01.01.2024" } },
    { label: "Supplier: INV-A-002 (Learning Test)", data: { id: "INV-A-002", vendorName: "Supplier GmbH", date: "2024-01-18", totalAmount: 1190.0, currency: "EUR", lineItems: [{ description: "Widget", quantity: 40, price: 25.0, total: 1000.0, sku: "WIDGET-001" }], rawText: "Rechnungsnr: INV-2024-002\nLeistungsdatum: 15.01.2024" } },
    { label: "Supplier: INV-A-003 (PO Matching)", data: { id: "INV-A-003", vendorName: "Supplier GmbH", date: "2024-01-25", totalAmount: 595.0, currency: "EUR", lineItems: [{ description: "Widget Pro", quantity: 20, price: 25.0, total: 500.0 }], rawText: "Order No: PO-A-051" } },
    { label: "Supplier: INV-A-004 (Duplicate Base)", data: { id: "INV-A-004", vendorName: "Supplier GmbH", date: "2024-03-28", totalAmount: 1020.0, currency: "EUR", lineItems: [{ description: "Hardware", quantity: 1, price: 1020.0, total: 1020.0 }], rawText: "Purchase Order: PO-C-901" } },
    { label: "Parts AG: INV-B-001 (VAT Incl)", data: { id: "INV-B-001", vendorName: "Parts AG", date: "2024-02-05", totalAmount: 2400.0, currency: "EUR", lineItems: [{ description: "Spare Part", quantity: 200, price: 10.0, total: 2000.0 }], rawText: "VAT already included in total" } },
    { label: "Parts AG: INV-B-002 (VAT Strategy)", data: { id: "INV-B-002", vendorName: "Parts AG", date: "2024-02-20", totalAmount: 1785.0, currency: "EUR", lineItems: [{ description: "Bolts", quantity: 150, price: 10.0, total: 1500.0 }], rawText: "Prices incl. VAT" } },
    { label: "Parts AG: INV-B-003 (Currency Recovery)", data: { id: "INV-B-003", vendorName: "Parts AG", date: "2024-03-03", totalAmount: 1190.0, currency: null, lineItems: [{ description: "Nuts", quantity: 500, price: 2.0, total: 1000.0 }], rawText: "Currency: EUR" } },
    { label: "Parts AG: INV-B-004 (Semantic Duplicate)", data: { id: "INV-B-004", vendorName: "Supplier GmbH", date: "2024-03-28", totalAmount: 1020.0, currency: "EUR", lineItems: [{ description: "Hardware", quantity: 1, price: 1020.0, total: 1020.0 }], rawText: "Duplicate submission of PA-7810" } },
    { label: "Freight: INV-C-001 (Skonto)", data: { id: "INV-C-001", vendorName: "Freight & Co", date: "2024-03-01", totalAmount: 1166.2, currency: "EUR", lineItems: [{ description: "Transport charges", quantity: 1, price: 1000.0, total: 1000.0 }], rawText: "2% Skonto if paid within 10 days" } },
    { label: "Freight: INV-C-002 (SKU Mapping)", data: { id: "INV-C-002", vendorName: "Freight & Co", date: "2024-03-10", totalAmount: 1190.0, currency: "EUR", lineItems: [{ description: "Seefacht / Shipping", quantity: 1, price: 1000.0, total: 1000.0 }], rawText: "Service: Seefacht" } },
    { label: "Freight: INV-C-003 (Surcharges)", data: { id: "INV-C-003", vendorName: "Freight & Co", date: "2024-03-20", totalAmount: 1213.8, currency: "EUR", lineItems: [{ description: "Transport charges", quantity: 1, price: 1020.0, total: 1020.0 }], rawText: "Slight fuel surcharge applied" } },
    { label: "Freight: INV-C-004 (Final Edge Case)", data: { id: "INV-C-004", vendorName: "Freight & Co", date: "2024-03-28", totalAmount: 1213.8, currency: "EUR", lineItems: [{ description: "Transport charges", quantity: 1, price: 1020.0, total: 1020.0 }], rawText: "Note: delivery confirmation pending" } }
  ];

  const handleSelectSample = (e: any) => {
    const val = e.target.value;
    setSelectedSample(val);
    const sample = samples.find(s => s.label === val);
    if (sample) {
      setInvoiceInput(JSON.stringify(sample.data, null, 2));
    }
  };
            ...
            <AnimatePresence mode="wait">
              ...
              {activeTab === 'process' && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-12 gap-8 h-full">
                  ...
                  {/* Input Panel */}
                  <div className="lg:col-span-5 flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden h-[calc(100vh-12rem)]">
                    <div className="px-6 py-4 border-b border-slate-100 flex flex-col gap-3 bg-slate-50/50 flex-shrink-0">
                       ...
                       <select 
                        ...
                        className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold text-indigo-600 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer"
                       >
                         <option value="">-- SELECT DATASET SCENARIO --</option>
                         {samples.map(s => <option key={s.label} value={s.label}>{s.label}</option>)}
                       </select>
                    </div>
                    <textarea 
                      className="flex-1 w-full p-6 font-mono text-xs text-slate-600 bg-transparent border-none focus:ring-0 resize-none leading-relaxed"
                      value={invoiceInput}
